...
def readFiles(filePath, ref=''):
    ...
    if ref!='':
        if ref.RasterXSize != file_ds.RasterXSize or  ref.RasterYSize != file_ds.RasterYSize:
            warped_fn = os.path.join(resultats,"temp",os.path.basename(filePath).replace('.tif',"_warped.tif"))
            if not os.path.exists(os.path.dirname(warped_fn)) : os.mkdir(os.path.dirname(warped_fn))
            gdal.Warp (warped_fn,  file_ds, height= ref.RasterYSize, width = ref.RasterXSize )
            file_ds = gdal.Open(warped_fn)
...
def writeRaster(layer, out_path, reference_ds, nbands =1, dtype =gdal.GDT_Float32, driver='GTiff', ):
    ...
    if driver=='MEM':
        gdal.RasterizeLayer(segments_ds, [1], layer, options=['ATTRIBUTE=Id'])
    else:
...
def get_objects_MP(sub_list, img, segments, q1, q2):
    '''
...
def split_raster(raster_fn, div = 2):
    """ Fonction qui divise un raster en div x div tuiles """
    out_folder = os.path.join(resultats,'temp')
    if not os.path.exists(out_folder) : os.mkdir(out_folder)
    
    raster = gdal.Open(raster_fn)
    (xmin ,ref_xRes ,_ ,ymax ,_ ,ref_yRes) = raster.GetGeoTransform()
    print('GeoTrans is : ', raster.GetGeoTransform())
    xlen = ref_xRes * raster.RasterXSize
    ylen = -ref_yRes * raster.RasterYSize
    ...
    print(ref_xRes, ref_yRes, raster.RasterYSize, ylen)
    
    for i in range(div):
        for j in range (div):
            xmin = xsteps[i];  xmax = xsteps[i+1]
            ymin = ysteps[j+1]; ymax = ysteps[j]
            print(f' xmin : {xmin}')
            print(f' xmax : {xmax}')
            print(f' ymin : {ymin}')
            print(f' ymax : {ymax}')
            print()
            out_path = os.path.join( resultats , "temp", os.path.basename(raster_fn).replace(".tif", f'_tile{i+1}{j+1}.tif') )
            gdal.Warp(out_path ,raster,
                       outputBounds = (xmin ,ymin ,xmax ,ymax) )
    raster = None
    print(os.path.join( resultats , "temp", os.path.basename(raster_fn).replace(".tif", '_tile[0-9][0-9].tif')))
    return glob.glob(os.path.join( resultats , "temp", os.path.basename(raster_fn).replace(".tif", f'_tile[0-9][0-9].tif') ))